...
$(document).ready(function () {
    $('#edit').on("show.bs.modal", function (event) {  //#edit - id модального окна
        const button = $(event.relatedTarget);  //кнопка, которая вызывает модальное окно
        const id = button.data("id"); // Извлечение информации из аттрибута data-id="${user.id}" кнопки, это будет id редакируемого пользователя
